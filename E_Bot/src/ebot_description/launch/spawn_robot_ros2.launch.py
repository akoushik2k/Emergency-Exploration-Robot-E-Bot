...
def generate_launch_description():
    ...
    robot_localization_file_path = os.path.join(get_package_share_directory(
        package_description), 'config/ekf.yaml')
    slam_file_path = os.path.join(get_package_share_directory(
        package_description), 'config/slam.yaml')
    ...
    robot_state_publisher= Node(
        ...
        parameters=[{'use_sim_time': use_sim_time, 'robot_description': xml, 'rate': 60}],
        output="screen"
    )
    ...
    slam_node = Node(
        ...
        name='slam',
        output='screen',
        parameters=[slam_file_path, {'use_sim_time': use_sim_time}])
    ...
    rviz_config_dir = PathJoinSubstitution(
        [FindPackageShare("ebot_description"), "rviz", "slam.rviz"]
    )
    ...
    tf=Node(
        ...
        output='screen',
        arguments=['0.322389989983846', '0', '0.335120109521668', '0', '0', '0', '1', 'base_link',  'lidar_link'  ],
    )
